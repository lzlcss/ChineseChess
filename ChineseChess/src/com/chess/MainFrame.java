...
import com.chess.ChessBoard.ChessPoint;
...
public class MainFrame extends JFrame {
	...
	private NetWorkSetting nws = new NetWorkSetting(null);
	...
	boolean isMaster = false;//if player is the master side, the first attacker
	...
	// for network use:
	private Socket s = null; //socket
	private boolean isConnOver = false; // is connection still alive or not
	// for action listener;
	private ActionListener bn5 = null;
	...
	public JMenuBar createMenuBar(){
		JMenuBar menuBar;
		JMenu connect, gameControl, saveDoc;
		JMenuItem startConnect, disconnect, tryNewGame, tryPlayBack, saveStep, saveChat;
		
		connect = new JMenu("Connect");
		startConnect = new JMenuItem("Start Connection");
		startConnect.addActionListener(new StartConnect());
		connect.add(startConnect);
		disconnect = new JMenuItem("Disconnect Connection");
	...
	private class NewGame implements ActionListener {
        public void actionPerformed(ActionEvent e) {
        	if(isConnOver){
        		callNewGame = false;
        		String str = new String("NewGame-Apply " + "\n");
        		send(str);
        	}
        }
    } 
	...
	/**
	 * This method initializes jButton	
	 * 	The button is used to start connection
	 * @return javax.swing.JButton	
	 */
	private JButton getJButton5() {
		if (jButton5 == null) {
			jButton5 = new JButton();
			jButton5.setBounds(new Rectangle(254, 2, 123, 37));
			jButton5.setText("Start Connect");
			jButton5.addActionListener(new StartConnect());
		}
		return jButton5;
	}
	
	/**
	 * This ActionListener Class implement the function of starting the network connect, 
	 * connect to the server and start to receive information 
	 *by using the method in the getThread
	 */
	private class StartConnect implements ActionListener{
		public void actionPerformed(ActionEvent e) {
			nws.setModal(true);
			nws.setVisible(true);
			...
			isConnOver = false;
			connServer();
			stepShow.append("Network connected\n");
			jButton5.setText("Network connected");
			...
			if(getThread == null){//Start a new Thread to get the movement from the other side player
				getThread = new Thread(new ReceiveThread());
				getThread.start();
			}
        }	
	}
	...
	/**
	 * This ActionListener Class implement the function of disconnect, 
	 * it will disconnect the link between the server
	 *
	 */
	private class Disconnect implements ActionListener{
		public void actionPerformed(ActionEvent e) {
			// kill the thread
			disconnect();
			jButton5.addActionListener(bn5);
			jButton5.setText("Start Connection");
        }	
	}
	
	
	/**
	 * This ActionListener Class implement the function of sending Chat information to the server
	 * 
	 *
	 */
		...
		public void actionPerformed(ActionEvent e) {
			if(canGo){
				if(canPlayBack>=2){
					...
					String str = new String("PlayBack-Apply" + " "+"\n");
					send(str);
				}else{
					stepShow.append("The game has just start or there is less\n than two steps from last play back,\n every play back can only be taken\n two steps after the last one.\n");
				}
			}else{
				stepShow.append("You can take play back only when it is your turn!\n");
			}
        }		
	}
				...
				if (RedPlay) {
					...
					if (red.getIndex() != -1) {
						...
						if ((old_x != c.getIndX()) || (old_y != c.getIndY())) {
							...
							canGo = false;
							changeSideFlag();//Set information for the label on the upper-right corner to acknowledge the player to move
							String str = new String("Move-" + chessIndex + " "+ transportX + " " +transportY + " " + "\n");
			...
			// And the movement of the chess should following the rule of playing this chess.			
				if (RedPlay) {//If it is the term for red side to play
					...
					if ((old_x != r.getIndX()) || (old_y != r.getIndY())) {//eat and print code 
						...
						canGo = false;
						changeSideFlag();//Set information for the label on the upper-right corner to acknowledge the player to move
						String str = new String("Destroy-" + hitChess +" " +destroyedChess + " " + "\n");
	 ...
	 * @return: void
	 */
	public void getDestroy(ChessGroup cg1,ChessGroup cg2, int hitChess, int destroyedChess){
		...
		// And the movement of the chess should following the rule of playing this chess.		
			if (RedPlay) {//If it is the term for red side to play
				Chess r = (Chess)cg1.getChess().get(hitChess);//chess r is the chosen red chess
				Chess b = (Chess)cg2.getChess().get(destroyedChess);//chess b is the chosen black chess, in this term, the eaten one 
				//Set the index of the black chess as the target index 
				int x = b.getIndX();
				int y = b.getIndY();
				//set the index of the red chess as the source index
				...
				Chess r = (Chess)cg1.getChess().get(destroyedChess);//chess r is the chosen red chess, in this term, the eaten one 
				Chess b = (Chess)cg2.getChess().get(hitChess);//chess b is the chosen black chess
				//Set the index of the red chess as the target index 
				int x = r.getIndX();
				int y = r.getIndY();
				//set the index of the black chess as the source index
	...
	/**
	 * the method sent all the information to the server
	 */
	 public void send(String str)
	{
	 ...
	 /**
		 * the method set current information on the Text Area "SideFlag"
		 */
	 ...
	 /**
		 * the class receive all the information from the chess Server and do the operation according to the
		 * control on the current side
		 */
		 ...
		 public void run() {
				...
				try {
					...
					while (str != null && str.length() != 0)
					{
						...
						if(str3[0].equalsIgnoreCase("Master")){
							String[] str7 = str3[1].split(" ");
							if(str7[0].equalsIgnoreCase("true")){
								isMaster = true;
							}else{
								isMaster = false;	
							}
						}
						if(str3[0].equalsIgnoreCase("NewGame")){
							String[] str8 = str3[1].split(" ");
							if(str8[0].equalsIgnoreCase("Apply") && callNewGame){
									int isAccept = JOptionPane.showConfirmDialog(null, "Opposite applies to start new game, do you agree?");
									if(isAccept == 0){
							...
							}else if(str8[0].equalsIgnoreCase("Accept") ){
									JOptionPane.showMessageDialog(null, "Opposite has acceptted to start new game");
									if(isMaster == true){
										normalNewGame();
									}else{
										reverseNewGame();
									...
									callNewGame = true;
							}else if(str8[0].equalsIgnoreCase("Deny")){
									JOptionPane.showMessageDialog(null, "Opposite has denied to start new game");
									callNewGame = true;
							}							
						}
						if(str3[0].equalsIgnoreCase("Move")){
							String[] str4 = str3[1].split(" ");
							int chessIndex = Integer.parseInt(str4[0]);
							int transportX = Integer.parseInt(str4[1]);
							int transportY = Integer.parseInt(str4[2]);
							getMove(chessIndex, transportX, transportY);			
						}
						if(str3[0].equalsIgnoreCase("Destroy")){
							String[] str5 = str3[1].split(" ");
							int hitChess = Integer.parseInt(str5[0]);
							int destroyed = Integer.parseInt(str5[1]);
							getDestroy(red, black, hitChess, destroyed);		
						}
						if(str3[0].equalsIgnoreCase("PlayBack")){
							String[] str6 = str3[1].split(" ");
							if(str6[0].equalsIgnoreCase("Apply")){
								if(!canGo){
									int isAccept = JOptionPane.showConfirmDialog(null, "Opposite applies to play back, do you agree?");
									if(isAccept == 0){
										send("PlayBack-Accept" + " "+ "\n" );
										playback();								
									}else{
										send("PlayBack-Deny" + " " + "\n");
									}
								}	
							}else if(str6[0].equalsIgnoreCase("Accept")){
								if(canGo){
									JOptionPane.showMessageDialog(null, "Opposite has acceptted to start play back");
									playback();
								}
							}else if(str6[0].equalsIgnoreCase("Deny")){
								if(canGo){
									JOptionPane.showMessageDialog(null, "Opposite has denied to start play back");
								}
							}							
						}
						if(str3[0].equalsIgnoreCase("Chat")){
							chatShow.append(str3[1]);
						}
						if(str3[0].equalsIgnoreCase("Server")){
							String[] str6 = str3[1].split(" ");
							if(str6[0].equalsIgnoreCase("Disconnect")){
								jButton5.addActionListener(new StartConnect());
								jButton5.setText("Start Connect");
							}
						}
